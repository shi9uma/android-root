...
-   在 magisk 中，其操作是魔改 `boot.img` 变成 `boot-magisk-patch.img`，然后让 bootloader 在启动时装载 `boot-magisk-patch.img`，由于引导程序都被修改了，意味着可以在系统中增加 su 来使用 root 权限，也就实现了 Android 设备的 root（**基于用户空间**）
...
7.   在 platform-tools 目录下打开终端；
     ...
     3.   输入 `.\fastboot.exe oem lks` 可以查看设备（联发科，MTK）解锁状态（0/true 表示解锁，1/false 表示未解锁）；高通骁龙是 `.\fastboot.exe oem device-info`，
...
-   TWRP Recovery 模式（前提是需要手机型号有适配的 twrp 工具，而 note 11tpro 正好没有）
    ...
    3.   输入 `fastboot flash recovery .\twrp.img` 将 recovery 分区修改成 twrp 分区
